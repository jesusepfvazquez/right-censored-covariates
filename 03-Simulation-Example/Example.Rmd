...
As shown in the plot above, the probability estimates for when $X$ is observed (i.e., blue points) strongly resemble the oracle probabilities. In fact, the Pearson correlation between the oracle and the estimated probabilities is equal to `r round(cor(dat$myp_xz_mle[dat$D==1], dat$myp_xz[dat$D==1]),4)`. We now investigate the linear fit of $\hat{f}_{X|Z}(x,z)$  and the oracle distribution $f_{X|Z}$.
