...
In addition to these, we have the oracle probability of observing $X$ defined by $\pi_{X,Z}(w,z)$. The oracle values of this probability distribution become smaller the larger $W$ becomes since a larger a higher value of $X$ correspond to a higher probability that $X$ is censored. This relationship can be observed from the following plot:
...
The values for the probability defined by $(Y,Z)$ are similar to those defined by $(W,Z)$. In fact the correlation between these values is `r round(cor(dat$myp_xz, dat$myp_yz), 2)`. Thus, while not the same these probability values are close.
...
Now that we have generated the data, we will now estimate the linear regression parameters and their corresponding standard error estimates. Before that, we will need to calculate some probabilities from a uniform distribution in order to misspecify the probability of $X$ being observed. Estimates from one simulation can be obtained using the following code:
...
We can also recreate the figures, as presented in the Supplementary Materials.
...
For both, the marginal and conditional correlation given $Z$ between $(X,C)$ is non-zero. Therefore, our simulation scenario allow us to consider the case when $X$ and $C$ are not conditionally independent given $Z$. Moreover, under this simulation scenario, the percent censoring remains 50% (actual `r round(mean(dat$D), 2)`). The distribution of $(X,C)$ is as follows,
...
Now that we have generated the data, we will now estimate the linear regression parameters and their corresponding standard error estimates. Before that, we will need to calculate some probabilities from a uniform distribution in order to misspecify the probability of $X$ being observed. Estimates from one simulation can be obtained similarly using the following code. The only difference is that the "oracle" probability of observing $X$ and the estimators now account for the conditional dependence between $X$ and $C$ given $Z$:
...
Similar as for the independent case, we computed all simulations using the [UNC Longleaf cluster](https://help.rc.unc.edu/longleaf-cluster/), where up to 800 jobs can run simultaneously. These results, were then saved and analyzed individually. Instead of running 3,000 simulation locally, we have included the simulation results in this repository. We now load these and replicate the results presented in manuscript.
...
Yet, in practice, these distributions are typically not assumed known and must be estimated. Following our data simulation strategy, we estimate the parameters from these distributions by finding the set of parameters $\alpha = (\mu_{X,C|Z}, \Sigma_{X,C|Z})$ that maximize the conditional bivariate normal distribution of $f_{W,\Delta|Z}$. In other words, we find
...
As noted in the main manuscript, $\alpha$ is identifiable in our independent right-censored covariate setting as explained in the paper. We first proceed by estimating the regression parameters. First, we define a helper function that calculates the above
...
Using these estimates, we will compute (i) probability of $X$ being observed, (ii) define the density $f_{X|Z}$ for the estimation of augmented component in the ACC, MACC and AIPW estimators, and (iii) define the density $f_{X|Z}$ for the MLE.
...
As we can observe, the probability estimates for when $X$ is observed (i.e., $\delta=1$) resemble the oracle probabilities. In fact the pearson correlation between the oracle and the estimated probabilities is equal to `r round(cor(dat$myp_xz_mle[dat$D==1], dat$myp_xz[dat$D==1]),2)`. We now investigate the linear fit of $X$ and the estimated $f_{X|Z}$ distribution.
...
As plotted above, the estimated distribution of $X$ conditional on $Z$ results in near identical values when compared to the oracle values. We take a similar approach to computing the probabilities $\pi_{Y,Z}(y,z)$. After calculating the integral $\pi_{Y,Z}(X,Z) = \int \pi_{X,Z}(x,z) f_{X|Y,Z}(x,y,z) dx$, where $\pi_{X,Z}(x,z)$ are the estimated probabilities from the previous step, the estimated probabilities are closely linearly associated with the oracle probabilities as shown below.
...
Now that we have estimated the nuisance parameters, we will now use these to compute the probability of $X$ being observed (i.e., $\pi_{X,Z}(x,z)$ and $\pi_{Y,Z}(y,z)$), and to define the mean and standard deviation of $f_{X|Z}(x,z)$ for the MLE estimator.
...
Similarly, we computed all simulations using the [UNC Longleaf cluster](https://help.rc.unc.edu/longleaf-cluster/). These results, were then saved and analyzed individually. Instead of running 3,000 simulation locally, we have included the simulation results in this repository. We now load these and replicate the results presented in manuscript.
...
We can also create a similar figure,
...
To address the dependent covariate right-censoring problem, we adopt the data simulation scenario from Bartlette et al. (2024). However, we modify the simulation so that after calculating the missing data indicator, we compute $C = X - beta\{exp(Z), 1\}$. The function below can be used to generate the data.
...
The scatter plot shown below depicts the distribution of the outcome $y$ and $Z$, and how their values are associated with probability $\pi_{Y,Z}(y,z)$. That is, higher values of $Z$ and lower values of $y$ are associated with a higher probability of observing $X$.
...
We can generate 3000 datasets using the following code. Then we save it as `acc_example_stata.csv`.
...
Similarly, we computed all simulations using the [UNC Longleaf cluster](https://help.rc.unc.edu/longleaf-cluster/) and saved in this repository. We load the simulation results and replicate the results presented in manuscript.
